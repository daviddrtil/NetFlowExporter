...
// cd ~/Desktop/ISA
// make
// 1.1) ./flow -f /home/daviddrtil/Desktop/ISA/captured_packets/icmp_test.pcap
...
void nf_export(nf_cache_t *cache, nf_t *nf_to_export, args_t *args)
{
    ...
    nf_datagram->First = htonl(0);     // nf_data->first_sys
    nf_datagram->Last = htonl(0);      // nf_data->last_sys
...
void check_timers(nf_cache_t *cache, args_t *args, timeval_t *loaded_packet_systime)
{
    ...
    while (tmp_nf != NULL)
    {
        ...
        timeval_t *first_sys_systime = tmp_nf->data->first_sys;
        timeval_t *last_sys_systime = tmp_nf->data->last_sys;
        // todo misto slozitych prevodu se da prevest na 64bit cislo a porovnavat potom jednoduse integery

        // Compute difference between systime of first nf packet and systeom of loaded packet
        int active_diff_sec =  loaded_packet_systime->tv_sec - first_sys_systime->tv_sec;
        int active_diff_usec = loaded_packet_systime->tv_usec - first_sys_systime->tv_usec;
        if (active_diff_usec < 0)
        {
            active_diff_usec = USEC_IN_SECOND + active_diff_usec;
            active_diff_sec--;
        }

        // Compute difference between systime of first nf packet and systeom of loaded packet
        int inactive_diff_sec =  loaded_packet_systime->tv_sec - last_sys_systime->tv_sec;
        int inactive_diff_usec = loaded_packet_systime->tv_usec - last_sys_systime->tv_usec;
        if (inactive_diff_usec < 0)
        {
            inactive_diff_usec = USEC_IN_SECOND + inactive_diff_usec;
            inactive_diff_sec--;
        }

        // Export outdated netflow
        if (active_diff_sec >= args->active_interval || inactive_diff_sec >= args->inactive_interval)
        {
            // Neat print - DEBUG only! // todo smazat
            ...
            if (active_diff_sec >= args->active_interval)
                printf("Active\n");
            else
                printf("Inactive\n");

            nf_export(cache, tmp_nf, args);
        }
        tmp_nf = tmp_nf_next;
    }
}
...
void export_remaining_nfs(nf_cache_t *cache, args_t *args)
{
    nf_t *cur_nf = cache->first;
    nf_t *next_nf;
    while (cur_nf != NULL)
    {
        next_nf = cur_nf->next;
        nf_export(cache, cur_nf, args);
        cur_nf = next_nf;
    }
    free(cache);
}
...
{
    // todo smazat
    // int address_lenght = strlen(args->collector_address) + PORT_NUMBER_LENGHT + 2;
    // char *whole_collector_address = (char *)malloc(address_lenght);
    // if (whole_collector_address == NULL)
    // {
    //     fprintf(stderr, "Allocation of whole_collector_address failed.\n");
    //     exit(INTERNAL_ERROR);
    // }
    // sprintf(whole_collector_address, "%s:%s", args->collector_address, args->port_number);
    ...
    {
        check_timers(cache, args, &pcap_metadata.ts);
        ...
        tmp_data->dOctets = ip_header->ip_hl * WORDS2BYTES_SIZE;
        ...
        {
            // Has port number
            int data_offset = sizeof(struct ether_header) + tmp_data->dOctets;
        ...
        nf_t *existing_nf = get_nf(cache, tmp_data->key);   // Find netflow
        if (existing_nf != NULL)
        {
            // NetFlow exists, update its data
            existing_nf->data->last_sys->tv_sec = pcap_metadata.ts.tv_sec;
            existing_nf->data->last_sys->tv_usec = pcap_metadata.ts.tv_usec;
            existing_nf->data->tcp_flags |= tmp_data->tcp_flags;  // has effect only in case of TCP packets
            existing_nf->data->dOctets += tmp_data->dOctets;
            existing_nf->data->dPkts++;
            if (tcp_fin || tcp_reset)
            {
                // TCP connection was ended, netflow can be exported
                nf_export(cache, existing_nf, args);
            }
        }
        else
        {
            ...
            if (cache->nf_cnt + 1 > args->flow_cache_size)
            {
                ...
                nf_export(cache, cache->last, args);
            }

            nf_t *new_nf = nf_ctor();
            nf_key_t *nf_key = new_nf->data->key;
            nf_key->ip_protocol = tmp_data->key->ip_protocol;
            nf_key->src_ip = tmp_data->key->src_ip;
            nf_key->dst_ip = tmp_data->key->dst_ip;
            nf_key->src_port = tmp_data->key->src_port;
            nf_key->dst_port = tmp_data->key->dst_port;

            nf_data_t *nf_data = new_nf->data;
            nf_data->first_sys->tv_sec = pcap_metadata.ts.tv_sec;
            nf_data->first_sys->tv_usec = pcap_metadata.ts.tv_usec;
            nf_data->last_sys->tv_sec = pcap_metadata.ts.tv_sec;
            nf_data->last_sys->tv_usec = pcap_metadata.ts.tv_usec;
            nf_data->tcp_flags = tmp_data->tcp_flags;
            nf_data->dOctets = tmp_data->dOctets;
            nf_data->dPkts = 1;
            ...
            // todo potom smazat!
            char src_ip[IPV4_ADDRESS_LENGHT] = {'\0'};
            get_ipv4_address(nf_key->src_ip, src_ip);
            char dst_ip[IPV4_ADDRESS_LENGHT] = {'\0'};
            get_ipv4_address(nf_key->dst_ip, dst_ip);
            printf("Inserted nf with: %s:%d -> %s:%d.\n", src_ip, nf_key->src_port, dst_ip, nf_key->dst_port);
        }
    }   // while

    export_remaining_nfs(cache, args);
    cache = NULL;
